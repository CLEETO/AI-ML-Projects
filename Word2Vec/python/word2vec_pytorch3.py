...
class Word2vec:
    ...
    def load_saved_params(self):
        st = 0
        for f in glob.glob("params/torch3/saved_params_*.pth"):
            iter = int(op.splitext(op.basename(f))[0].split("_")[2])
            if (iter > st):
                st = iter
        if st > 0:
            checkpoint = torch.load("params/torch3/saved_params_%d.pth" % st)
    ...
    def save_params(self, iter, params):
        ...
        'state':torch.get_rng_state()
            ...
            torch.save(checkpoint, "params/torch3/saved_params_%d.pth" % iter)
    ...
    def word2vec_sgd_wrapper(self,word2vec_model, tokens,C, word2vec_gradient):
        batchsize = 1000
        total_cost=0
        epoch_progress=tqdm(obj.dataloader(batchsize),total=math.ceil(len(self.train_data)/batchsize),desc='Batch Progress',unit='batch',leave=False)
        for train_data,train_label in epoch_progress:   
            cost = 0.0
            for centerword, context in zip(train_data,train_label):
                ...
                cost += c
            cost /= batchsize
            epoch_progress.set_postfix(loss=f' {cost:.4f}')
            total_cost+=cost
        return total_cost
    ...
    def initialize(self):
        ...
        word_vectors0 = self.sgd(lambda : self.word2vec_sgd_wrapper(self.skipgram, self.vocab.__len__(),  self.window_size, self.negative_sampling),
                             0.01, 2, None, True, PRINT_EVERY=100)
