...
class Word2vec:
        ...
        with open('dataset_sentences.txt','r',encoding='utf-8') as file:
            for line in file:
                if first:
                    first=False
                    continue
                line=line.strip().replace('-',' ').lower().replace('-lrb-', '(').replace('-rrb-', ')').replace('ã©','é')
    ...
    def get_train_data(self):
        ...
        for line in self.load_data_sentences():
            for i in range(len(line)):
                ...
                #target_words=target_words+[-1]*(self.window_size*2-len(target_words))
                #print(target_words)
    ...
    def sampleTokenIdx(self):
        ...
        return self.sampleTable()[torch.randint(0, self.tablesize - 1,(1,))]

    
    def load_saved_params(self):
        st = 0
        for f in glob.glob("params/saved_params_*.pth"):
            iter = int(op.splitext(op.basename(f))[0].split("_")[2])
            if (iter > st):
                st = iter
        if st > 0:
            checkpoint = torch.load("params/saved_params_%d.pth" % st)
    ...
    def save_params(self, iter, params):
        ...
        'state':torch.get_rng_state()
            ...
            torch.save(checkpoint, "params/saved_params_%d.pth" % iter)
    ...
    def skip_softmax(self,input_index, target_index, K=10,skip_gram=True):      
        self.torch_model.train()
        self.optimizer.zero_grad()
        result=self.torch_model(input_index)
        #print(result.shape,target_index.shape[0])
        #total_loss=0
        #for target in target_index:
        # Calculate loss for each target index
        #    loss = self.loss(result.unsqueeze(0), torch.tensor([target], dtype=torch.long).to(device))
        #    total_loss += loss  # Accumulate the loss
        
        
        #average_loss = total_loss / target_index.shape[0]
        #average_loss.backward()
        result = result.repeat(target_index.shape[0], 1)  # Shape adjustment
        loss = self.loss(result, torch.tensor(target_index.to(device)))  # One loss computation for all context words
        loss.backward()
    ...
    def skipgram(self,current_word, context_words, tokens, word2vec_cost_grads):
        ...
        #print(current_word,context_words)
        cost = self.skip_softmax(current_word,context_words,True)
                #print(cost)
    ...
    def word2vec_sgd_wrapper(self,word2vec_model, tokens,C, word2vec_gradient):
        ...
        for train_data,train_label in tqdm(obj.dataloader(batchsize),total=math.ceil(len(self.train_data)/batchsize),desc='Batch Progress',unit='batch'):
            ...
            cost += c / batchsize
            total_cost+=cost
            tqdm.write(f'Batch cost: {cost:.4f}')
        tqdm.write(f'Total cost: {cost:.4f}')
        return total_cost
    ...
    def initialize(self):
        ...
        word_vectors0 = self.sgd(lambda : self.word2vec_sgd_wrapper(self.skipgram, self.vocab.__len__(),  self.window_size, self.skip_softmax),
