...
class StanfordSentiment:
    def __init__(self, path=None, tablesize = 1000000):
        ...
        #self.path = path
        #self._tokenizer = Tokenizer(BPE.from_file('gpt2_tokenizer_version3/vocab.json', 'gpt2_tokenizer_version3/merges.txt'))
        self._tokenizer = PreTrainedTokenizerFast(tokenizer_file="gpt2_tokenizer_version2/tokenizer.json")
    ...
    def tokens(self):
        ...
        for sentece in self.sentences():
            #print(sentece)
            #sentece = b" ".join(sentece).decode("utf-8").replace('-lrb-', '(').replace('-rrb-', ')').replace('ã©','é')
            #print(sentece)
            #sentece=' '.join([x for x in sentece])
            #encoded_sentece=self._tokenizer.encode(sentece)
            #encoded_sentece = b" ".join(encoded_sentece).decode("utf-8").replace('-lrb-', '(').replace('-rrb-', ')').replace('ã©','é')
    ...
    def allSentences(self):
        ...
        for sentece in sentences*30:
            ...
            #encoded_sentece=self._tokenizer.tokenize(sentece)
            words=[]
            #print(encoded_sentece)
            #print(sentece)

            #for word in encoded_sentece:#.tokens:
            for word in sentece:
                k=self._revtokens.index(word)
    ...
    def sentiment_labels(self):
        ...
        with open("datasets/stanford-sentiment-tree-bank/dictionary.txt", "r") as f:
            for line in f:
                line = line.strip()
        ...
        # Example: 12|0.33333
        with open("datasets/stanford-sentiment-tree-bank/sentiment_labels.txt", "r") as f:
            first = True
            for line in f:
                if first:
                    first = False
        ...
        for i in range(self.num_sentences()):
            ...
            full_sent = " ".join([x for x in sentence]).replace('-lrb-', '(').replace('-rrb-', ')').replace('ã©','é')
            #print(full_sent)
    ...
    def dataset_split(self):
        ...
        with open("datasets/stanford-sentiment-tree-bank/dataset_split.txt", "r") as f:
            first = True
            for line in f:
                if first:
                    first = False
