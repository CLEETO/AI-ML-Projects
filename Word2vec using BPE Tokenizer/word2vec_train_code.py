...
class Word2vec:
    ...
    def load_data_sentences(self):
        ...
        with open('dataset_sentences.txt','r',encoding='utf-8') as file:
            for line in file:
                ...
                self.sentences += [[w for w in line]]
    ...
    def load_saved_params(self):
        ...
        for f in glob.glob("params/saved_params_*.npy"):
            ...
            if (iter > st):
                st = iter
        if st > 0:
            with open("params/saved_params_%d.npy" % st, "rb") as f:
    ...
    def save_params(self, iter, params):
        with open("params/saved_params_%d.npy" % iter, "wb") as f:
    ...
    def negative_sampling(self,input_vector, output_vectors, target_index, K=15):
...
for word in visualizeWords:
    word=' '+word
    word=obj.tokenizer.tokenize(word)
    word_embedding=np.zeros(wordvectors.shape[1]).tolist()
    for sub_word in word:
        word_embedding+=wordvectors[obj.tokens_id[sub_word],:]
    visualizeVecs+=[word_embedding/len(word)]
